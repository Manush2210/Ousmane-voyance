<div x-data="{
    ...
    <nav class="bg-white ">
        <div class="max-w-screen-xl mx-auto px-4 py-3">
            ...
            <div class="md:hidden flex items-center justify-between">
                ...
                <div x-data="{ mobileCartOpen: false }" class="relative">
                    ...
                    <div x-show="mobileCartOpen" @click.away="mobileCartOpen = false" x-cloak
                        ...
                        wire:poll.visible
                        class="absolute right-0 mt-2 w-xs bg-slate-100 rounded-lg shadow-xl z-50">
                        <div class="p-4 max-h-96">
                            <h3 class="text-lg font-bold border-b border-gray-200 pb-2 mb-2">Votre Panier ({{$cartItemCount}})</h3>
            ...
            <div class="hidden md:flex justify-center  items-center space-x-6">
                ...
                <button @click="$dispatch('open-search-modal')" class="text-gray-600 hover:text-white hover:bg-red-700 p-2">
                    ...
                    <div class="relative">
                        ...
                        <a href="{{ !empty($search) && strlen($search) >= 2 ? route('search', ['search' => $search]) : '#' }}"
                           @if(empty($search) || strlen($search) < 2)
                               onclick="event.preventDefault(); alert('Veuillez saisir au moins 2 caractères');"
                           @endif
    ...
    <div x-data="{ isOpen: false }"
     @open-search-modal.window="isOpen = true"
            ...
            <div class="p-6">
                ...
                <div class="relative">
                    ...
                    <a href="{{ route('search') }}/{{ '${$wire.search}' }}"
                       :class="{ 'cursor-not-allowed opacity-50': $wire.search.length < 2, 'hover:bg-red-700': $wire.search.length >= 2 }"
                ...
                <p class="mt-3 text-sm text-gray-500">
                    <span x-show="$wire.search.length < 2">Veuillez saisir au moins 2 caractères pour lancer la recherche</span>
                    <span x-show="$wire.search.length >= 2">Appuyez sur Entrée ou cliquez sur l'icône pour rechercher</span>
                </p>
                ...
                <div class="mt-6" x-show="$wire.search.length < 2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button @click="$wire.search = 'cartes'; window.location.href = '{{ route('search') }}/cartes'"
                                class="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm hover:bg-red-50 hover:text-red-700">
                            Cartes
                        </button>
                        <button @click="$wire.search = 'tarot'; window.location.href = '{{ route('search') }}/tarot'"
                                class="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm hover:bg-red-50 hover:text-red-700">
                            Tarot
                        </button>
                        <button @click="$wire.search = 'pendule'; window.location.href = '{{ route('search') }}/pendule'"
                                class="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm hover:bg-red-50 hover:text-red-700">
                            Pendule
                        </button>
                        <button @click="$wire.search = 'oracle'; window.location.href = '{{ route('search') }}/oracle'"
                                class="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm hover:bg-red-50 hover:text-red-700">
                            Oracle
                        </button>
                    </div>
